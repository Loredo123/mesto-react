...
function AddPlacePopup({onClose, isOpen, onAddPlace}) {
    ...
    return (
        <PopupWithForm onSubmit={handleSubmit} onClose={onClose} isOpen={isOpen} title="Новое место" name="card" submit="Сохранить">
          ...
          <input value={place || ''} onChange={e => setPlace(e.target.value)} name="name" id="place-input" minLength="2" maxLength="30" type="text" placeholder="Название"
            className="form__input" required />
          <span className="form__error-message place-input-error"></span>
          <input value={link || ''} onChange={e => setLink(e.target.value)} name="link" id="url-input" type="url" placeholder="Ссылка на картинку" className="form__input"
            required />
